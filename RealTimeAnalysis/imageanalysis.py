...
def run_twitter_analysis():
    ...
    filepath = os.path.dirname(os.path.abspath(__file__))
    dataset_content = pd.read_excel(filepath+"/RealTimeUrl.xlsx",sheet_name="RealTimeUrls")
    dataset_stats = pd.read_excel(filepath+"/RealTimeUrl.xlsx",sheet_name="Stats")
    image_urls = dataset_content.iloc[:,0].values
    ...
    for uri in image_urls:
        ...
        i=i+1
        wb.sheets[0].range('B22').value = str(i)+" image/s analyzed, "+str(image_urls.size-i)+" image/s remaining."

    wb.sheets[0].range('B22').value = "I am processed your images..."
    ...
    writer.save()
    wb.sheets[0].range('B22').value = "Downloaded Analyzed file Successfully in Download/Image Analytics folder"